...
def send_verification_email(user, verify_url):
    """Отправляет письмо с подтверждением email через Gmail SMTP."""
    import smtplib
    from email.mime.multipart import MIMEMultipart
    from email.mime.text import MIMEText
    ...
    if not mail_sender or not mail_password:
        current_app.logger.warning("MAIL_SENDER / MAIL_PASSWORD not set — verification email not sent")
        return False

    try:
        msg = MIMEMultipart('alternative')
        msg['Subject'] = 'Подтверждение email — Vibe'
        msg['From'] = mail_sender
        msg['To'] = user.email
        ...
        html = (
            ...
            f'<p>Или перейди по ссылке: <a href="{verify_url}">{verify_url}</a></p>'
            f'<p>Ссылка действует 24 часа.</p>'
            f'<p>Если ты не регистрировался(ась) на Vibe — просто проигнорируй это письмо.</p>'
        )
        msg.attach(MIMEText(html, 'html', 'utf-8'))
        ...
        current_app.logger.error(f"SMTP verification error: {e}")
        return False
