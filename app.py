...
class Post(db.Model):
    ...
    comments = db.relationship('Comment', foreign_keys=[Comment.post_id], lazy='dynamic', cascade='all, delete-orphan')
...
def add_comment(post_id):
    ...
    post = Post.query.get_or_404(post_id)
    body = request.form.get('body', '').strip()
    reply_to_username = request.form.get('reply_to', '').strip()
    media_url = None
    media_type = None
    reply_to_id = None
    
    if reply_to_username:
        reply_user = User.query.filter_by(username=reply_to_username).first()
        if reply_user:
            reply_to_id = reply_user.id
    ...
    if body or media_url:
        try:
            comment = Comment(body=body or '', author=current_user, post=post, media_url=media_url, media_type=media_type, reply_to=reply_to_id)
